...
public class EncryptMovie {
	...
	private static void readProperties(String fullMovieName, String moviesConfig, String password) throws Exception {
		...
		try {
			...
			InputStream inputStream = new ByteArrayInputStream(ConfigReader.read(moviesConfig, movieName, password).toByteArray());
			if (inputStream == null) {
			...
			if(movieIntegrity != null) {
				...
				System.out.println("SIZE inputSize: "+inputBytes.length);
				...
				/*byte[] cipherText = new byte[cipher.getOutputSize(size)];
				int ctLength = cipher.update(inputBytes, 0, size, cipherText, 0);
				FileOutputStream outputStream = new FileOutputStream(encryptedFile);
				outputStream.write(cipherText);   // integrityData ??????? TODO
